...
float max_pairwise_difference(float *A, float *B, int m, int n, int rs, int cs)
{
...
int main(int argc, char *argv[])
{
    ...
    // use the root id to print the header on CSV file (dont want multiple headers)
    if (rid == root_id)
    {
    ...
    for (int size = min_size; size <= max_size; size += step_size)
    {
        ...
        */
        ...
        DISTRIBUTE_DATA_REF(m0, n0, A_seq, B_seq, C_seq, A_dist_ref, B_dist_ref, C_dist_ref);
        ...
        */
        ...
        {
            // verify the results
            float max_diff = max_pairwise_difference(C_dist_ref, C_dist_test, m0, n0, m0, 1);
